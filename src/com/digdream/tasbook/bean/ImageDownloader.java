...
public class ImageDownloader {  
    ...
    public void imageDownload(String url,ImageView mImageView,String path,Activity mActivity,OnImageDownload download){  
        ...
        String imageName = "";  
        if(url != null){  
            imageName = FileUtil.getInstance().getImageName(url);  
        }  
        ...
        //文件中也没有，此时根据mImageView的tag，即url去判断该url对应的task是否已经在执行，如果在执行，本次操作不创建新的线程，否则创建新的线程。   
            ...
            if(mImageView != null){  
                Log.i(TAG, "执行MyAsyncTask --> " + FileUtil.flag);  
                FileUtil.flag ++;  
                task.execute();  
                //将对应的url对应的任务存起来   
                map.put(url, task);  
        ...
        if(imageName != null){  
            ...
            try {  
                if(FileUtil.getInstance().hasSDCard()){  
                    real_path = FileUtil.getInstance().getExtPath() + (path != null && path.startsWith("/") ? path : "/" + path);  
                }else{  
                    real_path = FileUtil.getInstance().getPackagePath(mActivity) + (path != null && path.startsWith("/") ? path : "/" + path);  
                }  
        ...
        protected Bitmap doInBackground(String... params) {  
            ...
            if(url != null){  
                try {  
                    ...
                    String imageName = FileUtil.getInstance().getImageName(url);  
